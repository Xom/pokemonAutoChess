...
function playSound(key, volume = 1) {
    ...
    if (sound) {
        sound.currentTime = 0;
        sound.volume = (volume * (0, preferences_1.preference)("sfxVolume")) / 100;
        sound.play();
    }
}
function playMusic(scene, name) {
    ...
    try {
        ...
        const unsubscribeToPreferences = (0, preferences_1.subscribeToPreferences)(({ musicVolume }) => {
            music.setVolume(musicVolume / 100);
        });
        music.on("stop", unsubscribeToPreferences);
        scene.music = music;
        scene.sound.pauseOnBlur = !(0, preferences_1.preference)("playInBackground");
        scene.music.play({
            volume: (0, preferences_1.preference)("musicVolume") / 100,
            loop: true
        });
    }
    catch (err) {
