...
exports.getNbScarvesOnBoard = getNbScarvesOnBoard;
exports.getNbToolsOnBoard = getNbToolsOnBoard;
...
function getUnitPowerScore(pkm) {
    var _a;
    if (Pokemon_1.NonPkm.includes(pkm))
        return 0;
...
function getNbComponentsOnBoard(board) {
    return board
        ...
        .reduce((total, item) => {
        let nbComponents = 0;
        if (Item_1.Scarves.includes(item))
...
function getNbScarvesOnBoard(board) {
    return board
        .flatMap((pkm) => pkm.items)
        .reduce((total, item) => {
        let nbScarves = 0;
        if (Item_1.Scarves.includes(item))
            nbScarves = item === Item_1.Item.NULLIFY_BANDANNA ? 2 : 1;
        else if (item === Item_1.Item.SILK_SCARF)
            nbScarves = 1;
        return total + nbScarves;
    }, 0);
}
function getNbToolsOnBoard(board) {
    return board.flatMap((pkm) => pkm.items).filter((item) => (0, array_1.isIn)(Item_1.Tools, item))
        .length;
}
...
function validateBot(bot) {
    ...
    for (let stage = 0; stage < bot.steps.length; stage++) {
        try {
            ...
            const synergies = (0, synergies_1.computeSynergies)(board.map((p) => {
...
function validateBoard(board, stage, synergies) {
    const team = board
        .filter((p) => p.y > 0)
        .map((p) => (0, precomputed_pokemon_data_1.getPokemonData)(p.name))
        .filter((p) => p.passive !== Passive_1.Passive.INANIMATE);
    const items = getNbComponentsOnBoard(board);
    const maxItems = getMaxItemComponents(stage);
    const scarves = getNbScarvesOnBoard(board);
    const maxScarves = (0, synergies_1.getSynergyStep)(synergies, Synergy_1.Synergy.NORMAL);
    const nbToolsOnBoard = getNbToolsOnBoard(board);
    const nbMaxToolsOnBoard = (0, synergies_1.getSynergyStep)(synergies, Synergy_1.Synergy.ARTIFICIAL);
    const duos = Object.values(Pokemon_1.PkmDuos);
    const numberOfGoldBows = board
        .flatMap((p) => p.items)
        .filter((item) => item === Item_1.Item.GOLD_BOW).length;
    ...
    function removeUniqueThatEvolveToLegendary(p) {
        if ((0, precomputed_pokemon_data_1.getPokemonData)(Pokemon_1.PkmFamily[p.name]).rarity === Game_1.Rarity.UNIQUE &&
            (0, precomputed_pokemon_data_1.getPokemonData)(p.name).rarity === Game_1.Rarity.LEGENDARY) {
            return false;
        }
        return true;
    }
    ...
    const additionalUncommon = team.filter((p) => p.additional && p.rarity === Game_1.Rarity.UNCOMMON);
    ...
    if (stage < config_1.PortalCarouselStages[1] && uniques.length > 0) {
        throw new Error(`Unique Pokemons can't be played before stage ${config_1.PortalCarouselStages[1]}`);
    }
    if (stage < config_1.PortalCarouselStages[2] && legendaries.length > 0) {
        throw new Error(`Legendary Pokemons can't be played before stage ${config_1.PortalCarouselStages[2]}`);
    }
