...
function selectMatchups(state) {
    const players = (0, random_1.shuffleArray)((0, schemas_1.schemaValues)(state.players).filter((p) => p.alive));
    if (players.length <= 1)
        return [];
    const matchups = getAllPossibleMatchups(players);
    const matchupCombinations = completeMatchupCombination([], matchups, players);
    const matchupCombinationsCount = matchupCombinations.map((combination) => (0, array_1.sum)(combination.map((m) => m.count)));
    const lowestTotalCount = Math.min(...matchupCombinationsCount);
    const lowestTotalCountMatchupCombinations = matchupCombinations.filter((matchups, index) => matchupCombinationsCount[index] === lowestTotalCount);
    const matchupCombinationsDistance = lowestTotalCountMatchupCombinations.map((combination) => (0, array_1.sum)(combination.map((m) => m.distance)));
    const maxDistance = Math.max(...matchupCombinationsDistance);
    const mostDistantMatchups = lowestTotalCountMatchupCombinations.filter((matchups, index) => matchupCombinationsDistance[index] === maxDistance);
