...
class AfterGameRoom extends colyseus_1.Room {
    ...
    onCreate(options) {
        ...
        this.state = new after_game_state_1.default(options);
        if (options.players) {
            options.players.forEach((plyr) => {
                const player = new after_game_player_1.default(plyr.id, plyr.name, plyr.avatar, plyr.rank, plyr.pokemons, plyr.title, plyr.role, plyr.synergies, plyr.elo, plyr.games, plyr.gameStats);
