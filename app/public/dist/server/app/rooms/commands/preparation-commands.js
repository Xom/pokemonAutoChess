        ...
        return __awaiter(this, arguments, void 0, function* ({ client, message } = {}) {
            ...
            try {
                ...
                if (freeMemory < 0.1 * totalMemory) {
                    ...
                    const seeds = {};
                    if (this.state.gameMode == Game_1.GameMode.CUSTOM_LOBBY && message && message.seeds) {
                        const s = message.seeds['-1'];
                        if (s && /^\d+$/.test(s) && BigInt(s) < number_1.BIGTWO64) {
                            seeds['-1'] = s;
                        }
        ...
        try {
            if (((_a = client.auth) === null || _a === void 0 ? void 0 : _a.uid) === this.state.ownerId &&
                ...
                this.room.state.addMessage({
                    author: "Server",
                    authorId: "server",
                    payload: `Elo gain has been ${noElo ? "disabled" : "enabled"} for this game. Players need to ready again.`,
                    avatar: leader === null || leader === void 0 ? void 0 : leader.avatar
                });
                this.state.users.forEach((user) => {
                    if (!user.isBot)
        ...
        return __awaiter(this, arguments, void 0, function* ({ ownerId }) {
            try {
                ...
                if (user) {
                    ...
                    if (bots) {
                        bots.forEach((bot) => {
                            this.state.users.set(bot.id, new game_user_1.GameUser(bot.id, bot.name, bot.elo, 99, bot.avatar, true, true, "", types_1.Role.BOT, false));
                        });
                    }
                    this.room.updatePlayersInfo();
                }
